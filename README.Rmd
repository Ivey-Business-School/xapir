...
`get_timeline_post()` and `get_timeline_user()` will then take the list returned and clean the data into a tibble of
posts and users, respectively. 

### get_timeline_post()

`get_timeline_post()` uses the results from the API call and cleans the data to return a tibble of all tweets. 

```{r get-timeline-post, eval = FALSE}
...
`get_timeline_user()` uses the results from the API call and cleans the data to return a tiblle of all users. 
This includes accounts whose tweets were retweeted by the targeted account. 

```{r get-timeline-user, eval = FALSE}
