...
Package features include: 
  ...
  * See how many posts have been posted under a query using `get_post_count()`
...
`get_recent_posts()` allows for the user to search for posts using a query within the last 7 days. The 
following example uses the function to extract posts related to weddings and are not retweets nor replies.

``` {r get-recent-posts, eval = FALSE} 
...
`get_post_count()` allows for the user to see how many posts were posted within the last 7 days. The following
example uses the function to count the number of posts related to weddings and are not retweets nor replies.

``` {r get-post-count, eval = FALSE} 
