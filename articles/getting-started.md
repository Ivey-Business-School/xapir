...
URL, exactly. A missing callback shows up as “Something went wrong” on
X’s authorize page with no other detail. The sign-in needs an
interactive R session; from `Rscript`, set
`options(rlang_interactive = TRUE)` first.
