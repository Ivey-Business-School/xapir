...
Returns the users who follow a list via the [get list followers
endpoint](https://docs.x.com/x-api/lists/get-list-followers). Every user
...
A tibble with one row per follower: `list_id`, then the 24 columns
described in
[`extract_user()`](https://Ivey-Business-School.github.io/xapir/reference/extract_user.md),
from `created_at` to `user_id`. A list nobody follows gives the same
