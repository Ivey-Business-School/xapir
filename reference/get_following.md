...
Returns the users an account follows via the [get following
endpoint](https://docs.x.com/x-api/users/get-following). Every user
...
at `max_users`. The default `max_users = 1000` is about US\$10.00.
...
A tibble with one row per followed account and the 24 columns described
in
[`extract_user()`](https://Ivey-Business-School.github.io/xapir/reference/extract_user.md),
from `created_at` to `user_id`. An account that follows nobody gives the
