# Unpin List

Unpins a list from the signed-in account's lists via the [unpin list
endpoint](https://docs.x.com/x-api/users/unpin-list). Needs a user
token, so the first call opens a browser window to sign in. The request
is billed, so the function says what it costs before it sends anything.

## Usage

``` r
unpin_list(list_id)
```

## Arguments

- list_id:

  The id of the list to unpin, as a string.

## Value

Invisibly, the `data` list the API returns, `list(pinned = FALSE)`.
Stops with the API's message when the request is refused.

## Examples

``` r
if (FALSE) { # \dontrun{
unpin_list(list_id = "1146654567674912769")
} # }
```
