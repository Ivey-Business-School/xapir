...
#' via the [post lookup by post ID endpoint](https://docs.x.com/x-api/posts/post-lookup-by-post-id).
#'
...
#' @examples
#' \dontrun{
#' post <- get_post_by_id("1234567890123456789")
#' }
#' @export
get_post <- function(
  post_id,
    ...
    tryCatch(
      expr = {
        request(base_url = "https://api.x.com/2") |>
          req_url_path_append(
            endpoint = paste0("tweets/", post_id)
          ) |>
          req_url_query(
